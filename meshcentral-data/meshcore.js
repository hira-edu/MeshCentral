...
var TUNNEL_DEBUG = (process.env.MESHCENTRAL_TUNNEL_DEBUG === '1');
function tunnelDebug(message) { if (TUNNEL_DEBUG) { sendConsoleText(message); } }
                ...
                switch (data.type) {
                    ...
                    case 'tunnel':
                        ...
                        if (data.value != null) { // Process a new tunnel connection request
                            ...
                            tunnelDebug("TUNNEL-DEBUG: value=" + (data.value ? data.value.substring(0,80) : "NULL") + " xurl=" + (xurl ? xurl.substring(0,80) : "NULL") + " ServerUrl=" + (mesh.ServerUrl ? mesh.ServerUrl.substring(0,60) : "NULL"));
                            if (xurl != null) {
                                ...
                                tunnelDebug('TUNNEL-DEBUG: tunnel created index=' + index + ' url=' + (tunnel.url ? tunnel.url.substring(0,80) : 'NULL'));
                                tunnel.end();
...
{
    tunnelDebug('TUNNEL-DEBUG: onTunnelUpgrade fired, index=' + this.index + ' protocol=' + this.protocol + ' tcpport=' + this.tcpport);
    this.s = s;
...
function kvm_consent_ok(ws) {
    ...
    tunnelDebug('TUNNEL-DEBUG: kvm.pipe(ws) - connecting KVM output to browser tunnel');
...
function onTunnelData(data)
{
    ...
    if (this.httprequest.state == 0) {
        ...
        tunnelDebug('TUNNEL-DEBUG: onTunnelData state=0 data=' + (typeof data == 'string' ? data.substring(0,10) : 'binary(' + data.length + ')'));
        if ((data == 'c') || (data == 'cr')) {
            this.httprequest.state = 1;
            tunnelDebug('TUNNEL-DEBUG: relay connected, state=1');
        ...
        if (this.httprequest.protocol == 0) { // 1 = Terminal (admin), 2 = Desktop, 5 = Files, 6 = PowerShell (admin), 7 = Plugin Data Exchange, 8 = Terminal (user), 9 = PowerShell (user), 10 = FileTransfer
            ...
            tunnelDebug('TUNNEL-DEBUG: protocol set to ' + this.httprequest.protocol);

...
function terminal_windows_active_user_session_id(users)
{
    ...
    if (users.Active[0].SessionId == null) { return null; }
    return parseInt(users.Active[0].SessionId);
}
...
function terminal_userpromise_resolved(u)
{
    ...
    if (targetSessionId != null && isNaN(targetSessionId) == false)
    {
    ...
    {
        that.httprequest.connectionPromise._rej('Failed to start remote terminal session, no active Windows user session found');
    }
    this.that = null;
    that = null;
}
            ...
            agentUpdate_Start._selfupdate.on('response', function (img) {
                ...
                this._filehash.on('hash', function (h) {
                    ...
                    if (process.platform != 'freebsd' && process.platform != 'linux') {
                        ...
                        try { require('fs').unlinkSync(process.cwd() + agentfilename + '.update'); } catch (ex) { }
                        agentUpdate_Start._selfupdate = null;
                        return;
                    }

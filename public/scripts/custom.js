(function () {
  ...
  function renderPanel(container, opts) {
    ...
    [['Pipe Status', 'umhctl status'], ['List Processes', 'umhctl listProcesses'], ['Flow Contract', 'umhctl getFlowContract'], ['Capabilities', 'umhctl getCapabilities']].forEach(function (x) { query.appendChild(btn(doc, x[0], COLORS.query, function () { dispatch(state, x[1], 4); })); });
    ...
    injection.appendChild(btn(doc, 'Disable', COLORS.injection, function () { var cmd = umhPidCmd(state, 'disable', injectionPid.value); if (!cmd) return; state.clearError(); dispatch(state, cmd, 4); }));
    injection.appendChild(btn(doc, 'State', COLORS.injection, function () { var cmd = umhPidCmd(state, 'getInjectionState', injectionPid.value); if (!cmd) return; state.clearError(); dispatch(state, cmd, 4); }));
    panel.appendChild(injection);

    var injectionScope = group(doc, 'Target Scope', COLORS.injection);
    [['Inject All', 'umhctl injectAll'], ['Disable All', 'umhctl disableAll'], ['Clear Scope', 'umhctl clearTargetScope']].forEach(function (x) { injectionScope.appendChild(btn(doc, x[0], COLORS.injection, function () { state.clearError(); dispatch(state, x[1], 4); })); });
